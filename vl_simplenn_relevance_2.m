function res = vl_simplenn_relevance_2(net, x, dzdy, res, varargin)
...
%       res(i+1).x = vl_nnsoftmaxloss(res(i).x, l.class) ;
    case 'softmaxloss'
      ...
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
    case 'softmaxloss_relevance'
        opts.loss = 'softmaxlog_relevance';
        res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
    case 'mhinge'
      opts.loss ='mhinge';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;    
    case 'mshinge_relevance'
      opts.loss ='mshinge_relevance';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;  
    case 'mshinge'
      opts.loss ='mshinge';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;    
   ...
   case 'mshingesquared'
      opts.loss ='mshingesquared';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
   case 'mshingesquared_relevance'  
      opts.loss ='mshingesquared_relevance';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
    case 'euclidean'
      opts.loss ='euclidean';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
    case 'euclidean_relevance_prod'
      opts.loss ='euclidean_relevance_prod'; %
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ; 
    case 'euclidean_relevance_prodsup1'
      opts.loss ='euclidean_relevance_prodsup1';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ; 
    case 'euclidean_relevance_sum'
      opts.loss ='euclidean_relevance_sum';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
%     case 'euclidean_estimate'
%       opts.loss ='euclidean_estimate';
%       res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
%     case 'euclidean_estimate_relevance'
%       opts.loss ='euclidean_estimate_relevance';
%       res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
    case 'mshingecubed'
      opts.loss ='mshingecubed';
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ;
    case 'mshingecubed_relevance'
      opts.loss ='mshingecubed_relevance'; %
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], opts) ; 
   case 'relu'
...
%         res(i).dzdx = vl_nnsoftmaxloss(res(i).x, l.class, res(i+1).dzdx) ;
      case 'softmaxloss'
          opts.loss = 'softmaxlog';
        res(i).dzdx = vl_nnloss(res(i).x, l.class,   res(i+1).dzdx, opts) ;
      case 'softmaxloss_relevance'
        opts.loss = 'softmaxlog_relevance';
        res(i).dzdx = vl_nnloss(res(i).x, l.class,  res(i+1).dzdx, opts) ;
      case 'mhinge'
