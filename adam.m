...
% vectors
lr_t = lr * (((1 - opts.beta2^state.t)^0.5) / (1 - opts.beta1^state.t));%original
%lr_t = lr * (((1 - opts.beta2^state.t)^0.5) / (1 - opts.beta1^state.t).^2) ;

% Update `w`
w = w - lr_t * state.m ./ (state.v.^0.5 + opts.eps) ;%original
%w = w - lr_t * state.m.^2 ./ (state.v.^0.5 + opts.eps) ;
