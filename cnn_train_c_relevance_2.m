function [net, stats] = cnn_train_c_relevance_2(net, imdb, getBatch, varargin)
...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    ...
    case 'multiclass'
      ...
      if isempty(opts.errorLabels), opts.errorLabels = {'Gmean'} ; end  %opts.errorLabels = {'err_p1', 'err_n1'} ;
    case 'binary'
...
for epoch=start+1:opts.numEpochs
  ...
  if params.plotStatistics
    ...
    print(1, modelFigPath, '-dpdf', '-bestfit');%, 'resize','-bestfit') ;
  end
...
% function [n_err, p_err, n_total, p_total] = error_multiclass(params, labels, res)
% % -------------------------------------------------------------------------
% predictions = gather(res(end-1).x) ;
% [~,predictions] = sort(predictions, 3, 'descend') ;
% 
...
% end
% 
% p_labels_ind = find(labels==1); % label=1 correspond to positives
% n_labels_ind = find(labels==2); % label=2 correspond to negatives
% 
% n_error = ~bsxfun(@eq, predictions(:,:,:,n_labels_ind), labels(n_labels_ind)) ;%labels(n_labels_ind)=1
% p_error = ~bsxfun(@eq, predictions(:,:,:,p_labels_ind), labels(p_labels_ind)) ;%labels(p_labels_ind)=2
% 
% n_err = sum(sum(sum(mass(n_labels_ind) .* n_error(:,:,1,:)))) ;
% p_err = sum(sum(sum(mass(p_labels_ind) .* p_error(:,:,1,:)))) ;
% 
% n_total = length(n_labels_ind);
% p_total = length(p_labels_ind);

function [c_error, c_freq] = error_multiclass(params, labels, res)
% -------------------------------------------------------------------------
predictions = gather(res(end-1).x) ;
[~,predictions] = sort(predictions, 3, 'descend') ;
...
c= length(unique(params.imdb.images.labels)); % #classes starting from 1
%[c_freq ~] = histcounts(labels); % #instances within each class
c_error = [];
for i=1:c
    c_ind = find(labels==i); 
    c_freq(i)=length(c_ind); % frequency of each class
    error = ~bsxfun(@eq, predictions(:,:,1,c_ind), i) ;%labels(c_ind)=i (label itself) % get the first error '1'
    c_error(i)=  sum(sum(sum(mass(c_ind) .* error))) ;
end
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    res = vl_simplenn_relevance_2(net, im, dzdy, res, ...
                      'accumulate', s ~= 1, ...
                      'mode', evalMode, ...
                      ...
                      'weight_parameter', params.weight_parameter, ...
                      'pd_model', params.pd_model) ;

    % accumulate errors
    objective = sum(double(gather(res(end).x)));
    %[n_err, p_err, n_total, p_total]  = params.errorFunction(params, labels, res); %err is a vector containing p_err and n_err
    [c_error, c_freq] = params.errorFunction(params, labels, res); %err is a vector containing p_err and n_err
    error = sum([error, [objective; c_error(:)]],2) ;
    np_total = sum([np_total, c_freq(:)] ,2);
...
%       reshape(params.errorFunction(params, labels, res),[],1) ; ]],2) ;
  ...
  stats = extractStats(net, params, error, np_total) ; %stats = extractStats(net, params, error / num) ;
  %stats = sum([stats,[0 ; error; np_total]],2); % stats=[time;objective;err_n;err_p;n_total;p_total]
  stats.num = num ;
  stats.time = time ;
      ...
      if thisLR>0 || thisDecay>0
        ...
        if isempty(params.solver)
          ...
          state.solverState{l}{j} = vl_taccum(...
            ...
            -1, parDer) ; % dw_(t+1)= momentum* dw_(t) - 1* dw_(t+1)
...
% -------------------------------------------------------------------------
% function stats = extractStats(net, params, errors, np_total)
% % -------------------------------------------------------------------------
% stats.objective = errors(1) ; % err(2:end)= total err per class
% for i = 1:numel(params.errorLabels)
%  % stats.(params.errorLabels{i}) = errors(i+1) ;
%  stats.(params.errorLabels{i}) = errors(i+1)/np_total(i) ;%stats[time,objective;err_n;err_p]
%  %errorLabels{1} = postivies / errorLabels{2} = negatives / 
% end
...
% -------------------------------------------------------------------------
stats.objective = errors(1) ; % err(2:end)= total err per class
avg_err_per_class = errors(2:end)./ np_total; % vector k*1 where k=#classes
avg_err_per_class(isnan(avg_err_per_class))=0;
stats.(params.errorLabels{1}) = prod(1-avg_err_per_class).^(1/numel(np_total)); % .^(1/k) 
% for i = 1:numel(np_total) % for all k classes (labels) compute err per class
%  % stats.(params.errorLabels{i}) = errors(i+1) ;
%  stats.(params.errorLabels{i}) = errors(i+1)/np_total(i) ;%stats[time,objective;err_n;err_p]
% end
