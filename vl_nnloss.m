...
if  isempty(dzdy) %if nargin <= 2 || isempty(dzdy)
  switch lower(opts.loss)
    ...
    case 'softmaxlog_relevance'
      ...
      t_ci = (Xmax + log(sum(ex,3)) - x(ci)) .* (1 + opts.weight_parameter.* relevance_c) ;
        catch ME
            msg= 'error';
        end
    case 'mhinge'
    ...
    case 'mshinge_relevance'
      ...
      t_ci = max(0, (1 + opts.weight_parameter.* relevance_c) .* (1 - x(ci) + max(Q,[],3))) ;
    case 'mshingesquared'
      Q = x ;
      Q(ci) = -inf ;
    ...
    case 'mshingesquared_relevance'
      ...
      relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); % get the relevance for each label within c (1*1*28*10)
      %t = max(0, (1 + 1.* relevance_c) .* (1 - x(ci))) ;
      t_ci = max(0, (1 + opts.weight_parameter.* relevance_c) .* (1 - x(ci) + max(Q,[],3)).^2 ) ;
    case 'mshingecubed'
      Q = x ;
      Q(ci) = -inf ;
    ...
    case 'mshingecubed_relevance'
      ...
      relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); % get the relevance for each label within c (1*1*28*10)
      %t = max(0, (1 + 1.* relevance_c) .* (1 - x(ci))) ;
      t_ci = max(0, (1 + opts.weight_parameter.* relevance_c) .* (1 - x(ci) + max(Q,[],3)).^3 ) ;
    case 'euclidean'
    ...
    case 'euclidean_relevance_prodsup1'
       relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); 
       x(ci)=x(ci)-1; % either x=x-0 for i!=ci// or x=x-1 for i=ci 
       t_ci = (1+opts.weight_parameter .* relevance_c) .*(x).^2;%t_ci = (1+2.*relevance_c) .*(x).^ 2; % since 0<relevance_c<1, multiply by 2 to magnify
...
%         t_ci =(x).^ 2 + 2 .*2 .*relevance_c .* abs(x);
       relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); 
       x(ci)=x(ci)-1; % either x=x-0 for i!=ci// or x=x-1 for i=ci 
       t_ci = (1+ opts.weight_parameter .* relevance_c) .*(x).^ 2; % since 0<relevance_c<1, multiply by 2 to magnify
  ...
  switch lower(opts.loss)
    ...
    case 'mshinge_relevance'
      ...
      qi = offset + numPixelsPerImage * (q - 1) ;
      relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); 
      W = dzdy .* (1+ opts.weight_parameter .* relevance_c) .* ( x(ci) -  x(qi) < 1) ;% 50 instead of 2
      y = zerosLike(x) ;
      y(ci) = - W ;
      y(qi) = + W ;
    ...
    case 'mshingesquared_relevance'      
      ...
      W = dzdy .* (1+ opts.weight_parameter .* relevance_c) .* ( x(ci) -  x(qi) < 1) ; %W = dzdy .* (1+ 2 .* relevance_c) .* ( x(ci) -  x(qi) < 1) ;
      y = zerosLike(x) ;
      y(ci) = - W .* t_ci;
      y(qi) = + W .* t_ci;
    ...
    case 'mshingecubed_relevance'      
      ...
      t_ci = max(0, (1 - x(ci) + max(Q,[],3)).^2  ) ; %(1-^y_i_t+max ^y_i_k)^2
      relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); 
      W = dzdy .* (1+ opts.weight_parameter .* relevance_c) .* ( x(ci) -  x(qi) < 1) ; %W = dzdy .* (1+ 50 .* relevance_c) .* ( x(ci) -  x(qi) < 1) ;
      y = zerosLike(x) ;
      y(ci) = - W .* t_ci;
      y(qi) = + W .* t_ci;
    case 'euclidean'
    ...
    case 'euclidean_relevance_prodsup1'
        x(ci)=x(ci)-1; % either x=x-0 for i!=ci// or x=x-1 for i=ci 
       relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); 
       y = bsxfun(@times,dzdy , (1+ opts.weight_parameter .*relevance_c) .* (x)); %y = bsxfun(@times,dzdy , (1+2.*relevance_c) .* (x)); % since 0<relevance_c<1, multiply by 2 to magnify
    case 'euclidean_relevance_sum'
        relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); 
%         relevance_c = reshape(opts.pd_model(c), 1,1,1,[]); 
%         x(ci)=x(ci)-1; % either x=x-0 for i!=ci// or x=x-1 for i=ci 
